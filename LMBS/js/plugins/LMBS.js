...
LMBS_Settings.physicsDebugDraw = false;
...
//=============================================================================
/*
PIXI.Matrix.prototype.postMultiplyParameters = function(a, b, c, d, tx, ty){
	var a1 = this.a;
	var b1 = this.b;
	var c1 = this.c;
	var d1 = this.d;
...
function LMBS_SkewableSprite() { this.initialize.apply(this, arguments); }
LMBS_SkewableSprite.prototype = Object.create(Sprite.prototype);
LMBS_SkewableSprite.prototype.constructor = LMBS_SkewableSprite;


LMBS_SkewableSprite.prototype.updateTransform = function() {
  this.skewX = 0;
  this.skewY = 0.01;
	...
	worldTransform.a = 1;
	worldTransform.b = 0;
	worldTransform.tx = this.position.x + parentTransform.tx;
	worldTransform.c = 0;
	worldTransform.d = 1;
	worldTransform.ty = this.position.y + parentTransform.ty;
	worldTransform.postMultiplyParameters(
		Math.cos(-this.skewY) * this.scale.x,
		Math.sin(-this.skewX),
		Math.sin(this.skewY),
		Math.cos(this.skewX) * this.scale.y, 0, 0);
};
*/
...
/*
    ...
    */
    ...
    this._groundBody = new LMBS_BoxBody(1000, 10, 0, LMBS_ForceGroup.Ground);
  ...
  /*
  this._testSprite = new LMBS_SkewableSprite();
  this._testSprite.bitmap = ImageManager.loadBattleback1("Grassland");
  this._testSprite.x = 300;
  this._testSprite.y = 200;
  this._testSprite.scale.x = 0.15;
  this._testSprite.scale.y = 0.15;
  //this._testSprite.rotation = 1.0;
  viewportSprite.addChild(this._testSprite);
  */
    var width = Graphics.width;
    var height = Graphics.height;
    this._background1Visual = new LMBS_Visual();
    this._background1Visual.mainSprite = new Sprite_Base();
    this._background1Visual.mainSprite.bitmap = ImageManager.loadBattleback1("Grassland");
    this._background1Visual.mainSprite.anchor.x = 0.5;
    this._background1Visual.mainSprite.anchor.y = 0.5;
    this._background1Visual.mainSprite.setFrame(0, 0, width, height);
    this._background1Visual.addChild(this._background1Visual.mainSprite);
    this._background1Visual.setPosition(new LMBS_Vector3(15, 0, 0));
...
LMBS_SceneGraph.updateWorld = function() {
    ...
    // オブジェクトの削除チェック
    for (var i = this._objectList.length - 1; i >= 0; i--) {
        if (this._objectList[i].isDestroyed()) {
            this._objectList[i].onFinalized();
            this._objectList.splice(i, 1);
        }
    }
    // 物理ボディの削除チェック
    for (var i = this._physicsBodyList.length - 1; i >= 0; i--) {
        if (this._physicsBodyList[i].isDestroyed()) {
            this._physicsBodyList.splice(i, 1);
        }
    }

    // 物理オブジェクトのデバッグ描画
    if (LMBS_Settings.physicsDebugDraw) {
...
LMBS_Camera.prototype.updateMatrix = function() {
    ...
    if (this._mode == LMBS_Camera.Mode.TargetTracking) {
        ...
        var z = -Math.tan(fov) * Math.abs(hd);
        z = Math.min(-4, z);
        this.moveTo(player.position.x + hd, 0, z);
...
LMBS_Object.prototype.isDestroyed = function() {
    return false;
}
...
LMBS_Object.prototype.onFinalized = function() {
}
...
LMBS_Battler.prototype.setupComponents = function(x, y, width, height, forceGroup) {
    ...
    // 剛体
    this.mainBody = new LMBS_BoxBody(this._objectSize.x, this._objectSize.y, 1.0, this._forceGroup, ~this._forceGroup);
...
/** */
LMBS_Battler.prototype.getCoreBattler = function() {
    return null;
}

/** */
LMBS_Battler.prototype.getPosition = function() {
    return this.mainBody.getPosition();
};

/** */
LMBS_Battler.prototype.getDirection = function() {
...
/** override */
LMBS_Actor.prototype.getCoreBattler = function() {
    return this._actor;
}
...
/** override */
LMBS_Enemy.prototype.getCoreBattler = function() {
    return this._enemy;
}
 ...
 *
 */
function LMBS_DamageCollitionEmitter() { this.initialize.apply(this, arguments); }
LMBS_DamageCollitionEmitter.prototype = Object.create(LMBS_Object.prototype);
LMBS_DamageCollitionEmitter.prototype.constructor = LMBS_DamageCollitionEmitter;

/**
 * constructor
 * @param coreAction {Game_Action}
 */
LMBS_DamageCollitionEmitter.prototype.initialize = function(ownerBattlerObj, x, y, w, h, lifeFrameCount, coreAction) {
    LMBS_Object.prototype.initialize.call(this);
    this._ownerBattlerObj = ownerBattlerObj;
    this._lifeFrameCount = lifeFrameCount;
    this._coreAction = coreAction;

    var targetGroup = 0x0000;
    if (this._coreAction.isForOpponent()) {
        // 相手
        targetGroup = ~this._ownerBattlerObj.forceGroup();
    }
    else if (this._coreAction.isForFriend()) {
        // 味方
        targetGroup = this._ownerBattlerObj.forceGroup();
    }

    this._body = new LMBS_BoxBody(w, h, -1, LMBS_ForceGroup.Emitter, targetGroup);
    this._body.setPosition(x, y);
};

/** override */
LMBS_DamageCollitionEmitter.prototype.isDestroyed = function() {
    return this._lifeFrameCount <= 0;
};

/** override */
LMBS_DamageCollitionEmitter.prototype.onUpdate = function() {

    var self = this;
    this._body.forEachContacts(function(thisBody, otherBody){
        otherBody.applyImpulse(self._ownerBattlerObj.getDirection() * 0.5, 0.5);
    });

    this._lifeFrameCount--;
};
